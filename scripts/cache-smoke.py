#!/usr/bin/env python3
"""Exercise a real HTTP cache across stylesheet releases; no routing mocks."""
import asyncio
...
class Assets(HTMLParser):
    def __init__(self):
        super().__init__();self.styles=[];self.script=''
    def handle_starttag(self,tag,attrs):
        attrs=dict(attrs)
        if tag=='link' and attrs.get('rel')=='stylesheet':self.styles.append(attrs['href'])
        if tag=='script' and attrs.get('type')=='module':self.script=attrs.get('src','')
async def exercise(browser,name,mobile,assets):
    ...
    seed='<!doctype html><html><head>'+''.join(f'<link rel="stylesheet" href="{escape(p)}">' for p in paths)+'</head><body>Previous release cache</body></html>'
    class Handler(MediaRangeHandler):
        def log_message(self,*args):pass
        def do_GET(self):
            state['requests'].append(self.path);url=urlsplit(self.path)
            if url.path.startswith('/__qa_css_cache_seed__'):self.send_body(seed.encode(),'text/html','no-store')
            elif state['old'] and not url.query and url.path in paths:
                index=paths.index(url.path)
                self.send_body(f':root {{ --qa-stale-style-{index}: 1; }}'.encode(),'text/css','public, max-age=31536000')
            else:
                try:super().do_GET()
    ...
    page.on('response',lambda r:failures.append(f'{r.status} {r.url}') if r.status>=400 and r.request.resource_type in ('image','stylesheet','script','font') else None)
    ...
    try:
        for visit in (1,2):
            ...
            for index in range(len(paths)):
                ...
                assert value=='1',f'Old CSS not cached/applied for {index}'
        counts=Counter(state['requests'])
        assert all(counts[p]==1 for p in paths),f'Test missed actual HTTP cache reuse: {counts}'
        ...
        # This contract now includes the entire site's images and playable videos,
        # not just the current hero. Still require actual readiness, never skip it.
        ...
        for index in range(len(paths)):
            value=await page.evaluate('(i)=>getComputedStyle(document.documentElement).getPropertyValue(`--qa-stale-style-${i}`).trim()',index)
            assert value=='',f'Stale cached stylesheet {index} leaked into new page'
        ...
        result['video_preparation']=await page.evaluate('window.__portfolioVideoCache || null')
        ...
        await context.close();server.shutdown();server.server_close();thread.join(timeout=2)
    return result
async def main():
    OUTPUT.mkdir(parents=True,exist_ok=True)
    assets=Assets();assets.feed((DIST/'index.html').read_text())
    match=re.search(r'/_app/(release-\d+-\d+|local)/site\.js$',assets.script)
    assert match,f'Missing versioned JS entry: {assets.script}'
    release=match[1];style_paths=[urlsplit(href).path for href in assets.styles]
    ...
    for file in DIST.rglob('*.html'):
        document=Assets();document.feed(file.read_text());assert document.styles,f'No stylesheets in {file}'
        assert all(parse_qs(urlsplit(href).query).get('release')==[release] for href in document.styles),f'Unversioned/mismatched CSS in {file}'
    results=[]
    async with async_playwright() as tool:
        for engine,name,mobile in ((tool.chromium,'chromium-desktop',False),(tool.webkit,'webkit-mobile',True)):
            browser=await engine.launch()
            try:
                result=await exercise(browser,name,mobile,assets);results.append(result);print(json.dumps(result,ensure_ascii=False),flush=True)

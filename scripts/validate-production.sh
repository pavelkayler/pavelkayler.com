...
set -euo pipefail
reject_match() {
  # A negated pipeline alone is exempt from bash errexit; fail explicitly.
  if grep "$@"; then
    echo 'Forbidden production pattern detected' >&2
    return 1
  fi
}
...
reject_match -q '\.html</loc>' dist/sitemap.xml
...
reject_match -q 'rel="canonical"' dist/404.html
reject_match -q 'property="og:image"' dist/404.html
reject_match -q 'data-seo-schema' dist/404.html
grep -q 'NotFoundPage' src/app/router.tsx
reject_match -q '<Navigate' src/app/router.tsx
...
test -s dist/assets/styles/portfolio-layout.css
test -s dist/assets/fonts/site-icons.woff2
test -s dist/assets/fonts/font-awesome/fontawesome-light-300.woff2
...
manifest = json.loads(Path('src/content/media-manifest.json').read_text())
...
test ! -f src/content/structured.ts
reject_match -R 'dangerouslySetInnerHTML' src --include='*.ts' --include='*.tsx'
reject_match -R 'generated/structured' src --include='*.ts' --include='*.tsx'
reject_match -q 'html: string' src/content/page-metadata.ts
...
test -d media/images
test -d media/video
reject_match -R 'wfolio\.ru/' src/content scripts --include='*.ts' --include='*.json' --include='*.mjs' --include='*.py'
...
python3 scripts/validate-file-names.py --include-dist

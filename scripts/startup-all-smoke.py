#!/usr/bin/env python3
"""One startup gate, then every page offline. Real HTTP/cache, not route mocks."""
import asyncio
        ...
        class Handler(SimpleHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if owner.tail+'-' in path:
                    owner.requested.set()
                    if owner.failure:
                        self.failed=True; self.send_error(503,'Deliberate last photograph failure'); return
                    if not owner.release.wait(120):
                        self.failed=True; self.send_error(504,'Test release did not arrive'); return
                try: super().do_GET()
...
async def cold(browser,name,mobile,entry):
    ...
    try:
        ...
        assert await asyncio.to_thread(server.requested.wait,45),'Last Portraits image was not included in startup'
        ...
        result['checks'].append('Holding a final album photo blocks the single startup gate even when entering Home/Contacts')
        ...
        expected_videos=sum(bool(a.get('cover') and a['cover'].get('videoSrc')) for a in ALBUMS.values())
        assert sum(t['id'].startswith('video:') for t in tasks)==expected_videos
        ...
        result['checks'].append('Every route module, page image, zoom resource and full cover video completed before reveal')
        # Disconnect after readiness: no missed resource can silently load during traversal.
        await context.set_offline(True); stamp=time.monotonic()
        for route,album in ALBUMS.items():
            ...
            await checks.fast_scroll(page,count)
            ...
            await page.evaluate('window.pswp.next()')
            await page.wait_for_function('window.pswp?.currIndex === 0'); await viewer_ready(page)
            ...
            if album.get('cover') and album['cover'].get('videoSrc'):
                ...
                assert (await video.get_attribute('src') or '').startswith('blob:'),'Video needs another network request'
            ...
            result['checks'].append(f'{route}: all {count} photos ready at reveal, rapid scroll, final/first zoom image, cover and Back work offline')
        ...
        photos=page.locator('#home-main .picture-section img')
        assert await photos.count()==8
        ...
        assert not [p for t,p in server.requests if t>stamp],'Late network requests'
        assert not errors,errors
        assert not failed,failed
        result['checks'].append('All primary pages and Home viewer offline: no route loader, no native snapshots, no late or failed network transfers')
        result['passed']=True
    except Exception as error:
        result['errors']=errors+[str(error)]; result['failed_requests']=failed
...
async def recovery(browser,partial_entry=False):
    server=Server(failure=True)
    context=await browser.new_context(viewport={'width':414,'height':896})
    page=await context.new_page(); page.set_default_timeout(90000)
    result={'name':'startup-explicit-partial-entry' if partial_entry else 'startup-tail-error-retry','passed':False}
    try:
        ...
        if partial_entry:
            await page.locator('#site-loader-continue').click()
            await page.wait_for_function("document.documentElement.dataset.siteLoadState === 'degraded'")
            assert await page.evaluate("window.__portfolioLoading.tasks.some(t=>t.state==='error')")
        else:
            server.failure=False; server.release.set()
...
async def main():
    ...
    async with async_playwright() as p:
        for engine,name,mobile,entry in ((p.chromium,'chromium-desktop',False,'/'),
            (p.chromium,'chromium-mobile',True,'/contacts/'),(p.webkit,'webkit-mobile',True,'/portraits/')):
            browser=await engine.launch()
            try:
                result=await cold(browser,name,mobile,entry); results.append(result)
                print(json.dumps(result,ensure_ascii=False),flush=True)
                if name=='chromium-desktop':
                    for partial_entry in (False,True):
                        result=await recovery(browser,partial_entry); results.append(result)
                        print(json.dumps(result,ensure_ascii=False),flush=True)

#!/usr/bin/env python3
"""One startup gate, then every page without an origin. Real HTTP/cache, not route mocks."""
import asyncio
...
async def cold(browser,name,mobile,entry):
    ...
    try:
        ...
        assert await asyncio.to_thread(server.requested.wait,45),'Last Portraits image not included in startup'
        ...
        result['checks'].append('Holding a final album photo blocks the only startup gate even on Home/Contacts entry')
